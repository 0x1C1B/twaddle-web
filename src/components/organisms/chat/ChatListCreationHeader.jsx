 ...
 */
export default function ChatListPrivateChatCreationHeader({onNewChat, onReturn}) {
  return (
    <div className="border-b border-slate-300 px-2">
      <Tab.Group>
        <div className="flex py-3 justify-between items-center h-16 space-x-2 pr-2">
          ...
          {/* TODO Add tabs for different chat types, for now tab header is hidden */}
          <Tab.List className="flex w-fit space-x-1 rounded-xl bg-gray-200 p-1 hidden">
            <Tab
              className={({selected}) =>
        ...
        <div className="pb-3 space-y-2 p-2">
          <Tab.Panels className="mt-2">
            <Tab.Panel>
              <PrivatChatCreationForm onNewChat={onNewChat} />
            </Tab.Panel>
          </Tab.Panels>
        </div>
      </Tab.Group>
...
ChatListPrivateChatCreationHeader.propTypes = {
